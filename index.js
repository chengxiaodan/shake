...
Putao.Index.prototype = {
		...
		bindEvent: function() {
			...
			$("#audioPlay").on('click', function() {
				if(audio.paused) {
					audio.play();
					this.style.backgroundImage = "url(/images/pc_img/mu_on.png)";
				} else {
					audio.pause();
					this.style.backgroundImage = "url(/images/pc_img/mu_off.png)";
				}
			});
			...
			$('#code').click(function() {
				$(this).animate({
					...
					$('#main').removeClass('b slideInRight animated');
					$('#ready_main').show();
					$('#gift').hide();
					$('#timer').hide();
					$('#pic_code').show();
			...
			$('#ready').click(function() {
				...
				$('#ready_main').show();
				$('#winer').hide();
				$('#timer').css('backgroundImage', "url(/images/pc_img/num/8.png)");
				$('#gift').hide();
				$('#pic_code').hide();
				$('#code').hide();
				$('#code_static').show();
		...
		timer: function() {
			...
			self.time = setInterval(function() {
				...
				if(num == 0) {
					...
					$('#timer').css('backgroundImage', "url(/images/pc_img/num/" + num + ".png)");
				}
			}, 1000);
		},
		playPause: function() {
		...
		_pause: function() {
			...
			$('#video').bind('ended', function() {
				...
				$('#ready_main').hide();
				$('#code_static').show();
				$('#main').addClass('b slideInRight animated');
				$('#replay').hide();
				$('#winer').show();
